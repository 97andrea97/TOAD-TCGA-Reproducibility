...
def collate_MIL_mtl_concat(batch):
	...
	sex = torch.LongTensor([item[2] for item in batch]) #modified: sex = torch.LongTensor([item[3] for item in batch])
...
def generate_split(cls_ids, val_num, test_num, samples, n_splits = 5,
	...
	for i in range(n_splits):
		...
		for c in range(len(val_num)):
			...
			if (label_frac == 1) or (label_frac == 100) or (label_frac == 1.0):
...
def plot_training_history(train_loss, val_loss, train_error, val_error, val_auc, filename="training_history.png"):
    """
    Plots the training history with three subplots:
    ...
    - val_error: List of validation error values.
    - val_auc: List of validation AUC values.
    - filename: The filename to save the plot (default: 'training_history.png').
    """
    ...
    # Plot 2: Training Error vs. Validation Error
    axs[1].plot(epochs, train_error, 'bo-', label='Training Error')
    axs[1].plot(epochs, val_error, 'ro-', label='Validation Error')
    axs[1].set_title('Training and Validation Error')
    axs[1].set_xlabel('Epochs')
    axs[1].set_ylabel('Error')
    axs[1].legend()
    
    # Plot 3: Validation AUC
    axs[2].plot(epochs, val_auc, 'go-', label='Validation AUC')
