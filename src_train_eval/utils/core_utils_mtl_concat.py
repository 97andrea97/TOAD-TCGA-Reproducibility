...
class EarlyStopping:
    ...
    def __init__(self, patience=20, stop_epoch=50, verbose=False):
        ...
        self.val_loss_min = np.inf # np.Inf depending on the numy version
    ...
    def save_checkpoint(self, val_loss, model, ckpt_name):
        ...
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model to: {ckpt_name}')
...
def train(datasets, cur, args):
    ...
    all_epochs_cls_train_loss = []
    all_epochs_cls_train_error = []
    all_epochs_cls_val_loss = []
    all_epochs_cls_val_error = []
    all_epochs_cls_val_auc = []

    start_training = time.time()
    for epoch in range(args.max_epochs):
        print(f'starting epoch {epoch}, {time.strftime("%H:%M:%S")}')
        cls_train_loss, cls_train_error = train_loop(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn)
        stop, cls_val_loss, cls_val_error, cls_val_auc = validate(cur, epoch, model, val_loader, args.n_classes, 
            early_stopping, writer, loss_fn, args.results_dir)
        
        # save results:
        all_epochs_cls_train_loss.append(cls_train_loss)
        all_epochs_cls_train_error.append(cls_train_error)
        all_epochs_cls_val_loss.append(cls_val_loss)
        all_epochs_cls_val_error.append(cls_val_error)
        all_epochs_cls_val_auc.append(cls_val_auc)

        try:
            plot_training_history(all_epochs_cls_train_loss, all_epochs_cls_val_loss, all_epochs_cls_train_error, all_epochs_cls_val_error, all_epochs_cls_val_auc, filename=os.path.join(args.results_dir, "training_val_history.png"))
        except:
            print('plot_training_history has some problems')
        print(f'results until epoch {epoch}: \nall_epochs_cls_train_loss={all_epochs_cls_train_loss} \nall_epochs_cls_train_error={all_epochs_cls_train_error} \nall_epochs_cls_val_loss={all_epochs_cls_val_loss} \nall_epochs_cls_val_error={all_epochs_cls_val_error} \nall_epochs_cls_val_auc={all_epochs_cls_val_auc}')

        time_difference = time.time() - start_training
        hours, remainder = divmod(time_difference, 3600)
        minutes, seconds = divmod(remainder, 60)
        print(f"From beginning to epoch {epoch} time: {int(hours)} hours, {int(minutes)} minutes, {int(seconds)} seconds")
        if stop: 
            break
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   
    ...
    for batch_idx, (data, label, sex) in enumerate(loader):
        ...
        if (batch_idx + 1) % 1000 == 0:
            print('batch {}, cls loss: {:.4f}, '.format(batch_idx, cls_loss_value) + 
                'label: {}, sex: {}, bag_size: {}'.format(label.item(), sex.item(), data.size(0)))
    ...
    if epoch ==0:
        ...
        # Get the next time in seconds since the epoch
    end_epoch = time.time()
    ...
    # Print the results
    print(f"Epoch {epoch} time: {int(hours)} hours, {int(minutes)} minutes, {int(seconds)} seconds")
    ...
    return cls_train_loss, cls_train_error
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None):
    ...
    if early_stopping:
        ...
        if early_stopping.early_stop:
            ...
            return True, cls_val_loss, cls_val_error, cls_auc

    return False, cls_val_loss, cls_val_error, cls_auc
