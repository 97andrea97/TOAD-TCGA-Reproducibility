...
def save_splits(split_datasets, column_keys, filename, boolean_style=False):
	...
	df.to_csv(filename)
	print(f"saving split {filename}: {df.head(10)}")
...
class Generic_WSI_MTL_Dataset(Dataset):
		...
		if self.patient_strat:
			...
			for split in range(len(ids)): 
				for idx in ids[split]:
					...
					if split == 0:
						train_patient_ids.append(case_id)
					elif split == 1:
						val_patient_ids.append(case_id)
					elif split == 2:
						test_patient_ids.append(case_id)

			self.train_ids, self.val_ids, self.test_ids = slide_ids[0], slide_ids[1], slide_ids[2]
			
			#added:
			# Check for overlaps between the sets
			train_val_overlap = set(train_patient_ids).intersection(set(val_patient_ids))
			train_test_overlap = set(train_patient_ids).intersection(set(test_patient_ids))
			val_test_overlap = set(val_patient_ids).intersection(set(test_patient_ids))
			...
			print(f'train_patient_ids into set_splits function :\n{train_patient_ids}')
			print(f'val_patient_ids into set_splits function:\n{val_patient_ids}')
			print(f'test_patient_ids into set_splits function:\n{test_patient_ids}')

			print(f'len(train_patient_ids) into set_splits function :\n{len(train_patient_ids)}')
			print(f'len(val_patient_ids) into set_splits function:\n{len(val_patient_ids)}')
			print(f'len(test_patient_ids) into set_splits function:\n{len(test_patient_ids)}')

			
			print(f'len(set(train_patient_ids)) into set_splits function :\n{len(set(train_patient_ids))}')
			print(f'len(set(val_patient_ids)) into set_splits function:\n{len(set(val_patient_ids))}')
			print(f'len(set(test_patient_ids)) into set_splits function:\n{len(set(test_patient_ids))}')

		else:
			message='are you sure you want patient_strat set to False??'
	...
	def get_split_from_df(self, all_splits=None, split_key='train', return_ids_only=False, split=None):
		...
		if len(split) > 0:
			...
			# modified
			df_slice = self.slide_data[mask]

			print(f"dropping nan from {df_slice.head(10)}")

			# Check if 'site' column exists before trying to process it
			if 'site' in df_slice.columns:
				if df_slice['site'].isna().all():
					df_slice = df_slice.drop(columns=['site'])
					print("The 'site' column was removed as all values were NaN.")
				else:
					print("The 'site' column contains some non-NaN values and was not removed.")
			else:
				print("The 'site' column does not exist in the DataFrame.")

			# Drop remaining rows with any NaN and reset index
	...
	def return_splits(self, from_id=True, csv_path=None):
		if from_id:
			...
			csv_file_debug = pd.read_csv(csv_path)
			
			pd.set_option('display.max_columns', None)  # Show all columns
			pd.set_option('display.max_rows', None)     # Show all rows
			pd.set_option('display.width', 1000)        # Adjusts width to avoid line wrapping

			print(f'found csv file: {csv_file_debug.head(10)}]')
	...
	def test_split_gen(self, return_descriptor=False):
		...
		for task in range(len(self.label_dicts)):
			if len(list(self.label_dicts[task].keys()))==0:
				if task == 1:
					print("skipping site task")
			else:
				index = [list(self.label_dicts[task].keys())[list(self.label_dicts[task].values()).index(i)] for i in range(self.num_classes[task])]
